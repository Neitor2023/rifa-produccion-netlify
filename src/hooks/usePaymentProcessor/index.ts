...
import { useReservationHandling } from './reservationHandling';
import { handleCompletePayment, ConflictResult } from './completePayment';
import { useNumberConflictManagement } from './numberConflictManagement';
import { usePaymentFlow } from './paymentFlow';
...
export function usePaymentProcessor({
  ...
  // Use the extracted hooks for conflict management
  const { 
    isConflictModalOpen, 
  ...
  } = useNumberConflictManagement(setSelectedNumbers, setIsPaymentModalOpen, refetchRaffleNumbers);
  
  // Use the extracted hooks for availability check
  ...
  // Use the extracted hooks for payment completion
  ...
  // Use the extracted hooks for reservation handling
  ...
  // Use extracted hook for participant management
  ...
  // Use the extracted number verification functionality
  const { verifyNumbersNotSoldByOthers } = usePaymentFlow(raffleId, debugMode);
  ...
  // Combined payment flow functions
  const handleProceedToPayment = async (numbers: string[], participantData?: ValidatedBuyerInfo, clickedButton?: string) => {
    console.log("💰 usePaymentProcessor: handleProceedToPayment called with:", {
      numbers: numbers.length, // Only log the count for privacy
  ...
  const handlePayReservedNumbers = async (numbers: string[], participantData: ValidatedBuyerInfo) => {
    console.log("💰 usePaymentProcessor: handlePayReservedNumbers called with:", {
      numbers: numbers.length, // Only log count for privacy
      hasParticipantData: !!participantData
    });
    ...
    try {
      ...
      if (debugMode) {
        console.log("usePaymentProcessor: Payment modal opened with validated data:", {
